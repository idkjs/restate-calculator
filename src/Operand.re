...
let readout =
  ...
  | AfterDecimal(sign, n2, n) =>
    signReadout(sign) ++ string_of_int(n) ++ "." ++ n2;
...
let update = (state, action) =>
  ...
  | (BeforeDecimal(s, n), Digit(d)) =>
    BeforeDecimal(s, n ++ string_of_int(d))
  | (BeforeDecimal(s, n), Decimal) => AfterDecimal(s, "", int_of_string(n))
  | (AfterDecimal(s, n2, n), Digit(d)) =>
    AfterDecimal(s, n2 ++ string_of_int(d), n)
  | (state, action) =>
    failwith(
