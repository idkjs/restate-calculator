...
module NaiveUpdater: Updater = {
  ...
  let update = (state, action) =>
    ...
    | Digit(n) =>
      ...
      let readout =
        if (state.decimal) {
          ...
          let integralPart =
            if (startReadout == "0." || startReadout == "-0.") {
              "";
            } else {
    ...
    | Percent =>
      let readout =
        Js.Float.toString(float_of_string(state.readout) /. 100.0);
    ...
    | Equal =>
      ...
      | Some((n, o)) => {
          ...
          readout:
            Js.Float.toString(
              evaluate(n, o, float_of_string(state.readout)),
            ),
    ...
    | Op(op) =>
      if (op == Subtract
        ...
        | Some((savedNumber, savedOp)) =>
          let result =
            evaluate(savedNumber, savedOp, float_of_string(state.readout));
          {
