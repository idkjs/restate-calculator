  ...
  let update = (state, action) =>
    ...
    | Digit(n) =>
      ...
      let readout = if state.decimal {
        ...
        let sign = if String.get(startReadout, 0) == '-' {
          "-"
        } else {
          ""
        }
      ...
      {...state, readout: readout, readoutEditable: true}
    | Percent =>
      let readout = Js.Float.toString(float_of_string(state.readout) /. 100.0)
      {...state, readout: readout, readoutEditable: false}
    | Equal =>
      switch state.pending {
      | None => {...state, readoutEditable: false, decimal: false}
      | Some((n, o)) => {
          pending: None,
          readout: Js.Float.toString(evaluate(n, o, float_of_string(state.readout))),
    ...
    | Op(op) =>
      if (
        op == Subtract &&
          (state.readoutEditable &&
          (state.readout == "0." || state.readout == "-0."))
      ) {
        {
          ...state,
          readout: if state.readout == "-0." {
            "0."
          } else {
            "-0."
          },
        ...
        | None => {
            ...state,
            pending: Some((float_of_string(state.readout), op)),
            readout: "0.",
            decimal: false,
          }
        | Some((savedNumber, savedOp)) =>
          ...
          {
            pending: Some((result, op)),
            readout: Js.Float.toString(result),
            readoutEditable: false,
            decimal: false,
          }
        }
